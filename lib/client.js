...
`
    ...
    function ComfySettings() {
      ...
      const save = () => {
        ...
        const payload = {
          baseUrl,
          loras: Object.fromEntries(loras.map((l) => [l.name, { triggerWords: l.triggerWords, mode: l.mode }])),
        }
        fetchJson('/save', { method: 'POST', body: JSON.stringify(payload) })
          .then((d) => {
            applyState(d)
            setMsg('已保存。生成时会把「写入提示词」的触发词拼到正向提示词前面。')
            setErr('')
          })
          .catch((e) => setErr(String(e.message || e)))
      ...
      return React.createElement('div', { className: 'cfy-page' },
        ...
        React.createElement('p', { className: 'cfy-intro' },
          '给模型用的连接和 LoRA 触发词。出图仍走对话里的工具；改节点去本机 ComfyUI。'),
        ...
        React.createElement('div', { className: 'cfy-card' },
          ...
          (state?.templates || []).map((t) => React.createElement('div', { className: 'cfy-tpl', key: t.id },
            `${t.id} · ${(t.media && t.media.out && t.media.out.join('+')) || (t.capabilities && t.capabilities.mode && t.capabilities.mode.join(',')) || ''} — ${t.title}`)),
        ...
        React.createElement('div', { className: 'cfy-card' },
          React.createElement('h3', null, 'LoRA 触发词'),
          React.createElement('p', { className: 'cfy-intro' },
            '按文件名保存。mode=写入提示词 会拼到正向提示词前；仅模型 只影响权重不写词（适合 turbo）。'),
          loras.map((l, i) => React.createElement('div', { className: 'cfy-lora', key: l.name + i },
          ...
          React.createElement('div', { className: 'cfy-row', style: { marginTop: 12 } },
            React.createElement('input', {
              style: { flex: 1 },
              value: extraName,
              placeholder: '再加一个 lora 文件名.safetensors',
            ...
            React.createElement('button', {
              ...
              onClick: () => {
                ...
                setLoras(loras.concat([{ name, triggerWords: '', mode: 'both', usedBy: [] }]))
                setExtraName('')
              },
            }, '添加'),
          ),
          React.createElement('div', { className: 'cfy-row', style: { marginTop: 12 } },
            React.createElement('button', { className: 'cfy-btn', disabled: busy, onClick: save }, '保存触发词'),
          ),
        ),
      )
    }
